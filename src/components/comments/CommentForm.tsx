    ...
    <form onSubmit={handleSubmit} className="mb-6">
      ...
      <div className="mt-3 flex justify-end items-center gap-2">
        ...
        {(parentId && onCancelReply) && (
             <button type="button" onClick={onCancelReply} className="px-4 py-2 rounded-md text-sm text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500" disabled={isSubmitting}>
               返信をキャンセル
             </button>
        )}
        {(isEditing && onCancelEdit) && (
             <button type="button" onClick={onCancelEdit} className="px-4 py-2 rounded-md text-sm text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500" disabled={isSubmitting}>
               編集をキャンセル
             </button>
        )}
        <button
