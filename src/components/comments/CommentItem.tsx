...
export function CommentItem({ comment, onReply, onEdit, onDelete }: CommentItemProps) {
  ...
  const isOwnComment = currentUser && user && currentUser.id === user.id;
  ...
  return (
    <div className="flex space-x-3">
      <div className="flex-shrink-0">
        {user?.avatar_url ? (
          <img src={user.avatar_url} alt={user.full_name || 'Avatar'} className="h-10 w-10 rounded-full object-cover" />
        ) : (
      ...
      <div className="flex-1">
        <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-3">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center space-x-2">
              <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(created_at).toLocaleString('ja-JP')}</p>
              {isOwnComment && (
                <div className="flex items-center">
                  {onEdit && <button onClick={handleEdit} className="p-1 text-xs text-gray-500 hover:text-gray-700">編集</button>}
        ...
        <div className="mt-1">
          <button onClick={() => onReply(commentId)} className="text-xs text-blue-600 hover:underline dark:text-blue-400">返信する</button>
        </div>
